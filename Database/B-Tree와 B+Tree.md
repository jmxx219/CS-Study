...
- 리프가 아닌 모든 노드들은 최소 2개 이상의 자식을 가져야 #
...
#### B+Tree 장점
- 리프 노드를 제외하고 데이터를 담아두지 않기 떄문에 메인 메모리를 더 확보해 더 많은 key를 수용할 수 있음
...
- 특시 데이터베이스 시스템은 디스크 기반이기 때문에 I/O 작업에서 상당한 시간을 소모함
- 디스트가 느린 이유
...
- 메인 메모리는 주요 저장소로, 제한적인 공간 떄문에 모든 데이터를 저장할 수 없음
