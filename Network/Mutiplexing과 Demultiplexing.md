...
- IP 서비스 모델
    ...
    - 비신뢰적 서비스
        - 네트워크를 통해 데이터를 전송할 때 최선을 다하지만, 패킷 손실, 중복, 순서 변경등 문제를 보장하지 않음
...
**사용 이유**
- 소켓 주소(`IP주소 + 포트번호`)를 통해 응용 프로세스 주소를 구분할 수 있음

- IP주소를 통해 네트워크에서 통신하는 각 호스트를 식별 함
    - 호스트는 NIC을 통해 네트워크에 연결 됨
    - 각 NIC는 고유한 IP주소를 할당 받음
        - `NIC (Network Interface Card)`: 컴퓨터의 내부에 위치하는 하드웨어 장치로, 이를 통해 컴퓨터가 네트워크에 연결되고 통신할 수 있음

    > 따라서 IP주소는 NIC를 통해 연결된 호스트들을 식별한다.
