...
즉, 제네릭은 **동일한 클 래스 내부에 타입을 지정하는것이 아닌 외부에서 사용자에 의해 타입이 지정**되는 것을 의미한다.
...
## 제네릭 메서드
제네릭은 클래스뿐만 아니라 메서드에만 별도로 지정해 사용할수 있다.
...
### 제네릭 메서드 사용이유
제네릭 메서드가 왜 필요할까? -> **[정적(static)](https://github.com/jmxx219/CS-Study/blob/main/java/static%20class%EC%99%80%20static%20method.md) 메서드**를 선언할때 필요하기 때문이다.
...
배열이나 일반 타입과 달리 제네릭의 경우 캐스팅이 지원되지 않는다..!
```java
...
하지만 캐스팅이 지원되지 않는다면 유연하게 타입을 결정하기 위한 제네릭의 장점이 사라지게 된다.

따라서 <? extends T> , <? super T> 을 이용하여 캐스팅이 가능하도록 설정할수 있다.
