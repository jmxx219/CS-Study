...
- 필요성
    - 프로그램 성능저하를 방지하기 위해
        - 스레드 생성/소멸에 드는 비용을 작업(요청)이 들어올 때마다 하기엔 부담이 큼
...
- 동작 방식
    ...
    2. 요청이 오면 작업 큐에 넣고, 해당작업을 스레드 풀의 스레드가 많아 처리함
...
- 프로그래머가 잘못 사용하면 오류가 쉽게 발생함
- 자바와 같은 고급 언어에서 임계영역에 대해 스레드 안전한 연산을 지원해주는 것이 `모니터`(고급 언어 구조물)
