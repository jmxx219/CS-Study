...
: 프로세서에 의해 수행되는 프로그램의 단위로, 실행을 위해 시스템(커널)에 등록된 작업
...
- 제어 요소만 가지며  자원(코드, 데이터..)은 프로세스 내 다른 스레드와 공유
...
Process Control Block (프로세스 관리 블록)

: OS가 프로세스 관리에 필요한 정보를 저장한 구조체
...
스레드는 PCB를 가지지 않고 TCB(Thread Control Bolk)를 가진다.

TCB : 스레드의 상태와 관련된 정보를 저장하는 데이터 구조
...
- exec()
    ...
    : 새로운 프로그램으로 프로세스의 실행 내용 **교체**
    
    - 새로운 프로그램을 현재 프로세스의 주소 공간에 로드하고 실행하는데 사용
...
- OS에서 고아프로세스를 방지하기 위해 
부모 프로세스가 먼저 종료되면 자식 프로세스의 새로운 프로세스로 init(PID=1)설정
- init프로세스는 자식 프로세스가 종료될 때까지 기다린 후 wait함수를 호출
